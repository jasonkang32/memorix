...
private fun MemorixUnlockedNavHost(
    ...
    LaunchedEffect(pendingShareUris) {
        ...
        val poppedToWork = navController.popBackStack(Routes.Work, inclusive = false)
        if (!poppedToWork) {
            navController.navigateTopLevel(Routes.Work)
        }
        PendingMediaHolder.set(sharedUris)
        navController.navigate(Routes.WorkCompose) { launchSingleTop = true }
    }
